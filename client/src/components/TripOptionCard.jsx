...
function TripOptionCard(props) {
    const { index, selected, onClick, type, walk_to, walk_from, prebooking, departure, stop, arrival, price, discount, regular_price, ticket_share, alternative_share, safety_share, comfort_share, lumoTime, busTime, ticket_level, totalWalkingDistance, weather, weatherCondition, temperature, distance_threshold, temp_threshold, wait_threshold, walk_to_dist, walk_from_dist, requestedTime, total_ticket} = props
    const [showDetails, setShowDetails] = useState(false)
    const { setIsOpen, passengersNum } = useContext(AppContext);
    ...
    const segments = [
        ...
        ...((totalPassengers > 1 && !scenarioParam == 0 && !scenarioParam == 1 && totalPassengers - totalPassengersWithoutTicket != 0 && totalPassengers - totalPassengersWithTicket > 0) || ticket_share == 0 || scenarioParam == 2
