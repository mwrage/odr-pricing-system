...
function PriceDetails(props) {
    ...
    return (
      <div className="flex flex-col h-full">
        <Disclosure>
            <DisclosureButton className="inter-500 text-left pt-2 flex justify-between">
                Flexible Fahrt, flexibler Preis!
                <ChevronDown size={20} style={"-rotate-90"}/>
            </DisclosureButton>
            <div className="w-full h-0.5 bg-zinc-100 rounded-full my-2"></div>
            <DisclosurePanel className="hyphens-auto" lang="de">
                Was du zahlst, hängt von verschiedenen Faktoren deiner Fahrt ab. 
                Je nach Bedarf können sich diese auch reduzieren - damit du garantiert effizient mobil bleibst!
            </DisclosurePanel>
        </Disclosure>
        <div className="flex items-center justify-center w-full h-24">Kreisdiagramme hier</div>

        <div className="w-full flex">
            <button className={`flex justify-center border py-1 w-1/4 rounded-tl-md ${showExplanation === 1 ? "bg-amber-500 border-amber-600 border-b-transparent" : "bg-amber-500/50 border-transparent"}`} onClick={() => setShowExplanation(1)}>
                <Ticket1 size={20} />
            </button>
            <button className={`flex justify-center border py-1 w-1/4 ${showExplanation === 2 ? "bg-pink-500 border-pink-600 border-b-transparent" : "bg-pink-500/50 border-transparent"}`} onClick={() => setShowExplanation(2)}>
                <Bus1 size={20} color={"#FFFFFF"} />
            </button>
            <button className={`flex justify-center border py-1 w-1/4 ${showExplanation === 3 ? "bg-indigo-500 border-indigo-600 border-b-transparent" : "bg-indigo-500/50 border-transparent"}`} onClick={() => setShowExplanation(3)}>
            ...
            <button className={`flex justify-center border py-1 w-1/4 rounded-tr-md ${showExplanation === 4 ? "bg-sky-500 border-sky-600 border-b-transparent" : "bg-sky-500/50 border-transparent"}`} onClick={() => setShowExplanation(4)}>
                <CloudSun size={20} />
            </button>
        </div>
        <div className="mb-2">
        
            <div lang="de" className={`hyphens-auto border px-2 pb-2 rounded-b-md ${showExplanation === 1 ? "text-amber-500" : showExplanation === 2 ? "text-pink-500" : showExplanation === 3 ? "text-indigo-500" : "text-sky-500"}`}>
